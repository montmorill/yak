import { Bot, Context, HTTP, Type, Universal } from '@yak/core'
...
export namespace QQBot {
  ...
  export const Config: Schema<Config> = Schema.intersect([
    Type.Object({
      id: Type.String().description('机器人 id。').required(),
      secret: Type.String().description('机器人密钥。').role('secret'),
      type: Type.Union(['public', 'private'] as const).description('机器人类型。').required(),
      sandbox: Type.Boolean().description('是否开启沙箱模式。').default(false),
      endpoint: Type.String().role('link').description('要连接的服务器地址。').default('https://api.sgroup.qq.com/'),
      intents: Type.Bitset(QQ.Intents).description('需要订阅的机器人事件。'),
      retryWhen: Type.Array(Number).description('发送消息遇到平台错误码时重试。').default([]),
